...
func init() {
	android.RegisterModuleType("aosqp_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = aosqpExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = aosqpExpandVariables(ctx, dep_file)
	...
	cmd := aosqpExpandVariables(ctx, String(g.properties.Cmd))
