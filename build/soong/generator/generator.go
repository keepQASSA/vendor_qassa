...
func init() {
	android.RegisterModuleType("qassa_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = qassaExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = qassaExpandVariables(ctx, dep_file)
	...
	cmd := qassaExpandVariables(ctx, String(g.properties.Cmd))
